...
class NBeatsForecast:
    ...
    def train_nbeats(self):
        ...
        scaled_val = train_scaler.fit_transform(self.test_data)
        ...
        nbeats.fit(series=scaled_train, val_series=scaled_val, epochs=50, past_covariates=scaled_cov, val_past_covariates=scaled_cov, verbose=True)

        scaled_pred_nbeats = nbeats.predict(n=len(self.test_data), past_covariates=scaled_cov)
        pred_nbeats = train_scaler.inverse_transform(scaled_pred_nbeats)
        mae_nbeats = mae(self.test_data, pred_nbeats)

        dst_file = os.path.join(self.output_dir, 'nbeats.png')
        self.plot_results('NBEATS', self.test_data, pred_nbeats, dst_file)

        print(f'nbeats model: mae: {mae_nbeats:.2f} -> {dst_file}')
...
def main():
    ...
    nbeats = NBeatsForecast(FLAGS.input_csv, FLAGS.output_dir)
